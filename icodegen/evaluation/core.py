...
__all__ = ['get_mean_probs', 'find_parens', 'mean_dist_probs', 'token_taxonomy', 'non_wordy', 'get_error_rates',
           'ERROR_THRESHOLD', 'get_error_rates_df', 'ERROR_THRESHOLD', 'get_last_token_error_df', 'ERROR_THRESHOLD',
           'get_mean_cross_entropy', 'get_mean_probs', 'get_mean_cross_entropy_df', 'TYPES', 'save_results', 'evaluate']
...
def get_last_token_error_df(df: pd.DataFrame, model: Model, bs: int = 16, n: Optional[int] = None):
    ...
    for i in tqdm(range(0, n, bs), desc="Error Rates", total = (n // bs) + 1):
        # setup the padding for tokenizer (not sure if it improves performance)
        ...
        # model.tokenizer.no_padding()
        idx_last_ids = [len(x.ids) - 1 for x in model.tokenizer.encode_batch(batch)]
        # loop through the ids of the last token
        for bs_idx, tok_idx in enumerate(idx_last_ids):
            # check if the model predicted the correct token with a high enough probability
            if probs[bs_idx][tok_idx][inputs[bs_idx][tok_idx]] >= ERROR_THRESHOLD:
                ending.append(np.nan)
                continue
            # append the probability of the last token to the list
            ending.append(probs[bs_idx][tok_idx][inputs[bs_idx][tok_idx]])
