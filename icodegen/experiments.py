...
def _get_dist_probs(mthd: str, model: Model, opening: str, closing: str) -> Dict[int, float]:
    """
...
def mean_dist_probs(df: pd.DataFrame, model: Model, opening: Optional[str] = '<{>', closing: Optional[str] = '<}>', n: Optional[int] = None) -> pd.DataFrame:
    """
    ...
    df = df.iloc[:n].copy()
    dist_probs = df.code.apply(lambda mthd: _get_dist_probs(mthd, model, opening, closing)).values
    ...
    df_dist = pd.DataFrame({'dist': list(dist_probs.keys()), 'mean_prob': list(dist_probs.values())}).sort_values('dist').reset_index(drop = True)
    return df_dist
